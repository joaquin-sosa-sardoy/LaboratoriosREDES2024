...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name) # ACA OBTENGO LA IP gracias a la conexion socket BY HOST NAME del sv
    ...
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Creo la "conexion con los parametros por internet"
    s.connect((server_name, HTTP_PORT)) #Me conecto al puerto 80 (que es internet, del server_name)
