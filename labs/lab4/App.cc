...
void App::initialize() {
    ...
    // Initialize statistics -> Lo dice claramente je
    paquetesEnviados = 0;
    paquetesRecibidos = 0;
...
void App::finish() {
    // Record statistics
    recordScalar("Paquetes recibidos", paquetesEnviados.getCount());
    recordScalar("Paquetes enviados", paquetesEnviados.getCount()); //Creo que getCount() funciona para esto. No revisé bien pero la idea es esa.
